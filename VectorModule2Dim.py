...
def vectornumber():
    while True:
        try:
            ...
            print('\t**[The typed number]:',coeffic,'is a [valid integer number!] ]**\n')
            return coeffic
...
def introduce():
    while True:
        try:
            coeffic1 = float(input('\t(ª<ª) Enter the [new] value? '))
            print('\t**[ [The typed number]:',coeffic1,'is a [valid float number!] ]**\n')
            return coeffic1
        except ValueError as err:
            print('\n\t    ////')
...
# The [new] radiusenter() function created below will set the radius using any positive float number

def radiusenter():
    while True:
        try:
            def newRadius():
                r = float(input('\t/º<º\ Type a [new] value? '))
                return r
            coeffic2 = newRadius()
            ...
            print('\t**[The typed number]:',coeffic2,'is a [valid float number!] ]**\n')
            return coeffic2
        except ValueError as err:
            print('\n\t   _===_')
            print('\t    @<@ [Warning!]:',err)
...
# The function ResultantVector() upgrade is the end version: 4.1 
...
def ResultantVector():
    ...
    while i <= numb:
        ...
        def subRotine():
            ...
            instruction2()
            # The [new] radiusenter() function created to the any value of the [radius]
            rdvect = radiusenter() 
        ...
        # Was insert a [new] if test
        if i == 1:
            print("\n\t--[Warning]:")
            print("\t**[ THE [PREVIOUS COMPONENTS]: Vx(1) AND Vy(1) BEEN CALCULED; BUT ]**")
            print("\t**[ [NO EXIST RESULTANT(R) VECTOR] TO A ONLY VECTOR!]**\n")
            
        Rx = Vx + Rx
        Ry = Vy + Ry
        
        if i == numb and i != 1:    # upgrade if test
...
def scalarProduct(number):
    ...
    if number == 1 or number == 2:
       ...
       # Find the Scalars Procucts
        ...
        cossine = ( ScalarProd0 / ( sideA * sideB ) )
        radianAngle = math.acos(cossine)
        theta = math.degrees(radianAngle)
        radiantheta = math.radians(theta)
        sine = math.sin(radiantheta)
        height_h = sideA * sine
        ...
        print(f'\t-- The [sideA] relative as |vectorPQ|: {sideA:<.2f}')
        print(f'\t-- The [sideB] relative as |vectorPS|: {sideB:<.2f}')
        print(f'\t-- The [Perimeter(P)] is: {Perimeter:<.2f}','\n')
        print('\t-- The [terms] of the Scalar Product(vectorPSºPQ)] is:',vectorPSºPQ)
        print('\t-- The [Scalar Product(PQºPS)] is:',ScalarProd0)
        print(f'\t-- The Cossine of theta: {cossine:<.2f}') 
        print(f'\t-- The Theta angle is: {theta:<.2f}')
        print(f'\t-- The [Height(h)] relative as [sideA]=|vectorPQ| is: {height_h:<.2f}')
        print(f'\t-- The [Area(S)] of the [Parallelogram(PQRS)]: {EquatArea:<.2f}','\n')
