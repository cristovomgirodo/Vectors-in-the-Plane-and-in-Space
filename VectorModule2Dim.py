...
def ResultantVector():
    ...
    while i <= numb:
        if i == 1:
            print('\t**[Warning]**')
            print('\t- All the [Vectorials Components] will can be [positive] or [negative] or Zero!\n')
        ...
        while angle_degrees == 0 and radius == 0:
            ...
            print('\t º < º [ WARNING! ]')
            print('\t \ - /')
            print('\t** [ No exist none [vector] when the [coefficients]: angle_degrees = 0 and radius = 0 -- Ok! ] **')
            print('\t-- [ Enter with [New values] to the [coefficients]: angle_degrees = ? and radius = ? ] --\n')
            angle_degrees, angle_radians, radius = subRotine()
