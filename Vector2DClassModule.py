...
class ScalarProduct:
	...
	def getscalar_product(self):
		...
		if self.number == 1 or self.number == 2:
			...
			# Give the [coordinates] of the points: P, Q, and R vertics of trianglePQR
			coordinatespoint.number1 = 1
...
class Medianas:
	def __init__(self, compX = 0, compY = 0, medianaPM = 0, medianaQL = 0, medianaRK = 0, xK = 0, yK = 0, xL = 0, yL = 0,
	xM = 0, yM = 0, xP = 0, yP = 0, xQ = 0, yQ = 0, xR = 0, yR = 0, PointM = [], pointP = 0, PointL = [], pointQ = 0,
	PointK = [], pointR = 0, vectorPM = [], vectorQL = [], vectorRK = [] ):
		
		self.compX = compX 
		self.compY = compY 
		self.medianaPM = medianaPM
		self.medianaQL = medianaQL 
		self.medianaRK = medianaRK 
	...
	def findMedianaPM(self):
		...
		self.medianaPM = magnitudebivector.magbivector()
		answer.view_solution()
		print('\t-- The terms of the [vectorPM]:', self.vectorPM)
		print(f'\t-- The [mediana: PM] finded is {self.medianaPM:<.2f}','\n')
		return
		
	def findMedianaQL(self):
		midpoint.number = 2
		midpoint.getmid_point()
		...
		magnitudebivector.x = self.compX 
		magnitudebivector.y = self.compY
		self.medianaQL = magnitudebivector.magbivector()
		answer.view_solution()
		print('\t-- The terms of the [vectorQL]:', self.vectorQL)
		print(f'\t-- The [mediana: QL] finded is {self.medianaQL:<.2f}','\n')
		return
		
	def findMedianaRK(self):
		midpoint.number = 1
		midpoint.getmid_point()
		...
		magnitudebivector.x = self.compX 
		magnitudebivector.y = self.compY
		self.medianaRK = magnitudebivector.magbivector()
		answer.view_solution()
		print('\t-- The terms of the [vectorRK]:', self.vectorRK)
		print(f'\t-- The [mediana: RK] finded is {self.medianaRK:<.2f}','\n')
		return
		
medianas = Medianas()

class FindTheMedianas:
	'''Determine the medianas of the triangle(PQR)'''
	def __init__(self, number2 = 0):
		self.number2 = number2
		
	def find_the_medianas(self):
		print('\t\t- Key [1] to find the new medianaPM or')
		print('\t\t- Key [2] to find the new medianaQL or')
		print('\t\t- Key [3] to find the new medianaRK.\n')
		self.number2 = vector.enterIntegerData()
		if self.number2 == 1:
			medianas.findMedianaPM()
		elif self.number2 == 2:
			medianas.findMedianaQL()
		elif self.number2 == 3:
			medianas.findMedianaRK()
		else:
			textstring3.noexist_option()
		return

findthemedianas = FindTheMedianas()
